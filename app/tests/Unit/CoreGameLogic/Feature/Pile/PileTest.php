...
test('Cards can be drawn from piles', function () {
    $this->coreGameLogic->handle(
        $this->gameId,
        new SkipCard($this->players[0], $this->cardsBildung["buk0"]->id, $this->pileIdBildung)
    );
    ...
    $this->coreGameLogic->handle(
        $this->gameId,
        ActivateCard::create($this->players[0], $this->cardsBildung["buk1"]->id, $this->pileIdBildung)
    );
    ...
    $this->coreGameLogic->handle(
        $this->gameId,
        ActivateCard::create($this->players[1], $this->cardsFreizeit["suf0"]->id, $this->pileIdFreizeit)
    );
...
test('Shuffling resets draw counter', function () {
    $this->coreGameLogic->handle($this->gameId, new SkipCard($this->players[0], $this->cardsBildung["buk0"]->id, $this->pileIdBildung));
...
test('End of pile is reached', function () {
    ...
    $this->coreGameLogic->handle($this->gameId, new SkipCard($this->players[0], $topCard, $this->pileIdBildung));
    $topCard = PileState::topCardIdForPile($this->coreGameLogic->getGameEvents($this->gameId), $this->pileIdBildung);
    $this->coreGameLogic->handle($this->gameId, new SkipCard($this->players[0], $topCard, $this->pileIdBildung));
    $topCard = PileState::topCardIdForPile($this->coreGameLogic->getGameEvents($this->gameId), $this->pileIdBildung);
    $this->coreGameLogic->handle($this->gameId, new SkipCard($this->players[0], $topCard, $this->pileIdBildung));
...
test('End of pile is reached and its re shuffled', function () {
    ...
    $this->coreGameLogic->handle($this->gameId, new SkipCard($this->players[0], $topCard, $this->pileIdFreizeit));
});
