...
describe('FileInsolvenzForPlayer', function () {
    it('throws an error if the player has a positive balance', function () {
        ...
        $this->coreGameLogic->handle($this->gameId, FileInsolvenzForPlayer::create($this->players[0]));
    })->throws(
    ...
    it('throws an error if the player has an active insurance that could be cancelled', function () {
        ...
        $this->coreGameLogic->handle($this->gameId,
            ConcludeInsuranceForPlayer::create($this->players[0], InsuranceId::create(1)));
    ...
    it('works if player fulfills all requirements', function () {
        ...
        $guthabenBeforeInsolvenz = PlayerState::getGuthabenForPlayer($this->getGameEvents(), $this->players[0]);
        ...
        expect($playerFiledForInsolvenzEvent->playerId->equals($this->players[0]))->toBeTrue()
            ->and($playerFiledForInsolvenzEvent->year->equals(KonjunkturphaseState::getCurrentYear($eventsAfterInsolvenz)))
            ->and($playerFiledForInsolvenzEvent->resourceChanges->guthabenChange->value)->toEqual($guthabenBeforeInsolvenz->negate()->value);
    });
});
