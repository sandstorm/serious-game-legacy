...
<div class="konjunkturphase-end">
    ...
    @if ($moneySheetIsVisible)
        @if ($editIncomeIsVisible)
            <x-gameboard.moneySheet.income.money-sheet-income :money-sheet="$this->getMoneysheetForPlayerId($myself)" :player-id="$myself" :game-events="$this->gameEvents" />
        @elseif ($editExpensesIsVisible)
            <x-gameboard.moneySheet.expenses.money-sheet-expenses :money-sheet="$this->getMoneysheetForPlayerId($myself)" :player-id="$myself" :game-events="$this->gameEvents" />
        @else
            <x-gameboard.moneySheet.money-sheet :money-sheet="$this->getMoneysheetForPlayerId($myself)" />
        @endif
        <button wire:click="completeMoneysheetForPlayer()"
                type="button"
