...
<div x-data="{ playerListOpen: false }" x-trap.noscroll="playerListOpen"
    @class([
        'player-list',
    ])
    ...
    @foreach($players as $key => $player)
        ...
        >
            <button type="button" title="Spielerübersicht öffnen/schließen" class="button button--type-borderless" :aria-expanded="playerListOpen" id="player-details-trigger-{{ $key }}" aria-controls="player-details-{{ $key }}" x-on:click="playerListOpen = !playerListOpen">
                ...
                <span class="zeitsteine__label-mobile">
                    {{ $player->zeitsteine->mobileLabel }}
                    <x-gameboard.zeitsteine.zeitstein-icon :player-name="$player->name" :player-color-class="$player->playerColorClass" />
                </span>
                <ul class="zeitsteine">
