    ...
    <form wire:submit="takeOutALoan">
        <div class="take-out-loan">
            <div class="take-out-loan__form">
                ...
                <div class="form__group">
                    <label for="repayment">Rückzahlungssumme</label>
                    <x-form.textfield wire:model="takeOutALoanForm.repayment" id="repayment" name="repayment" type="number" step="0.1" />
                    <span>Rückzahlungssumme = Kreditsumme * (1 + Zinssatz/ 20).</span>
                    @error('takeOutALoanForm.repayment') <span class="form__error">{{ $message }}</span> @enderror
                </div>

                <div class="form__group">
                    <label for="repaymentPerRound">Rückzahlung pro Runde</label>
                    <x-form.textfield wire:model="takeOutALoanForm.repaymentPerRound" id="repaymentPerRound" name="repaymentPerRound" type="number" step="0.1" />
                    <span>Der Kredit wird innerhalb von 20 Jahren abbezahlt!</span>
                    @error('takeOutALoanForm.repaymentPerRound') <span class="form__error">{{ $message }}</span> @enderror
                </div>
            </div>
            <div class="take-out-loan__info">
                <p>
                    Aktuell {{ KonjunkturphaseState::getCurrentKonjunkturphase($gameEvents)->leitzins }}% Zins.
                </p>
                <p>
    ...
    <table>
        ...
        @foreach($loans as $loan)
            <tr>
                ...
                <td>{{ number_format($loan->getRepaymentAmount()->value, 2, ',', '.') }} €</td>
                <td>{{ number_format($loan->getRepaymentAmountPerRound()->value, 2, ',', '.') }} €</td>
                <td>xxx €</td>
            </tr>
        @endforeach
        <tr>
