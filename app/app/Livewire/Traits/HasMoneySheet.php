...
use Domain\CoreGameLogic\Feature\Moneysheet\Event\PlayerTookOutALoan;
...
trait HasMoneySheet
{
    ...
    public function takeOutALoan(): void
    {
        ...
        $guthaben = PlayerState::getGuthabenForPlayer($this->gameEvents, $this->myself)->value;

        // add error if the player tries to take out a loan that is higher than 10x their current guthaben
        if ($this->takeOutALoanForm->loanAmount > $guthaben * 10) {
            $this->takeOutALoanForm->addError('loanAmount',
                'Du kannst keinen Kredit aufnehmen, der höher ist als das 10-fache deines aktuellen Guthabens.');
            return;
        }

        $repaymentPeriod = PlayerTookOutALoan::REPAYMENT_PERIOD;
        $leitzins = KonjunkturphaseState::getCurrentKonjunkturphase($this->gameEvents)->leitzins;
        $correctRepayment = $this->takeOutALoanForm->loanAmount * (1 + $leitzins / $repaymentPeriod);
        if ($this->takeOutALoanForm->repayment !== $correctRepayment) {
            $this->takeOutALoanForm->addError('repayment',
                'Die Rückzahlung muss dem Kreditbetrag multipliziert mit dem Zinssatz geteilt durch 20 entsprechen.');
            return;
        }

        if ($this->takeOutALoanForm->repaymentPerRound !== $correctRepayment / $repaymentPeriod) {
            $this->takeOutALoanForm->addError('repaymentPerRound',
                'Die Rückzahlung pro Runde muss der Rückzahlungssumme geteilt durch 20 entsprechen.');
            return;
        }
        ...
        $this->coreGameLogic->handle($this->gameId, TakeOutALoanForPlayer::create(
            ...
            new LoanAmount($this->takeOutALoanForm->loanAmount),
