...
use Domain\CoreGameLogic\Feature\Spielzug\Command\DiscardCard;
...
trait HasCard
{
    ...
    public function renderingHasCard(): void
    {
        ...
        if (AktionsCalculator::forStream($this->gameEvents)->hasPlayerSkippedACardThisRoundAndNotPlayedOrDiscardedTheNext()) {
     ...
     * @return void
     */
    public function skipCard(string $category, string $pileId): void
    {
    ...
    /**
     * Player can not play the card if he cannot afford the top card on the pile.
     *
     * @param string $category
     * @return void
     */
    public function discardCard(string $category): void
    {
        $this->coreGameLogic->handle($this->gameId, new DiscardCard($this->myself, CategoryId::from($category)));
        $this->broadcastNotify();
    }
}
