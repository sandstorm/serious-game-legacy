...
use Domain\CoreGameLogic\Feature\Spielzug\Aktion\Validator\HasPlayerAnImmobilieToSellValidator;
...
class SellImmobilieAktion extends Aktion
{
    ...
    public function validate(PlayerId $playerId, GameEvents $gameEvents): AktionValidationResult
    {
        ...
        $validatorChain
            ->setNext(new HasPlayerAnImmobilieToSellValidator($immobilienCard->getImmobilienTyp()))
    ...
    public function execute(PlayerId $playerId, GameEvents $gameEvents): GameEventsToPersist
    {
        ...
        // get random immobilie from player which will be sold from same type
        $randomizer = new Randomizer();
        $immobilienOwnedByPlayer = $randomizer->shuffleArray(PlayerState::getImmoblienOwnedByPlayer($gameEvents, $playerId));
        $immobilieSold = null;
        foreach ($immobilienOwnedByPlayer as $immobilie) {
            if ($immobilie->getImmobilienTyp() === $immobilienCard->getImmobilienTyp()) {
                $immobilieSold = $immobilie->getId();
                break;
            }
        }
        ...
        return GameEventsToPersist::with(
            new ImmobilieWasSoldForPlayer(
                ...
                immobilienType: $immobilienCard->getImmobilienTyp(),
                immobilieSold: $immobilieSold
            )
        );
    }
}
