...
use Domain\CoreGameLogic\Feature\Spielzug\Aktion\Validator\HasAlreadyAnsweredThisWeiterbildungValidator;
...
class SubmitAnswerForWeiterbildungAktion extends Aktion
{
    ...
    {
        $validatorChain = new IsPlayersTurnValidator(); // Ist Spieler am Zug?
        $validatorChain
            ->setNext(new HasPlayerStartedAWeiterbildungThisTurnValidator()) // Hat Spieler Weiterbildung gestarted?
            ->setNext(new HasAlreadyAnsweredThisWeiterbildungValidator()) // Wurde schon auf Weiterbildung geantwortet?
            ->setNext(new SelectedAnswerForWeiterbildungExistsValidator($this->selectedAnswer)); // Die gewählte Antwort existiert auch
