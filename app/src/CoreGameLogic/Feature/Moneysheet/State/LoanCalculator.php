...
class LoanCalculator
{
    public static function getMaxLoanAmount(float $sumOfAlAssets, float $salary, float $obligations): MoneyAmount
    {
        if ($salary > 0) {
            ...
            return new MoneyAmount(round($salary * 5 + $sumOfAlAssets - $obligations, 2));
        } else {
            // if player has no job: limit is 80% of the assets - obligations
            return new MoneyAmount(round($sumOfAlAssets * 0.8 - $obligations, 2));
        }
    }
    ...
    {
        return new MoneyAmount(round($openRepayment * 1.01, 2));
    }
}
