...
class InvestmentPriceState
{
    ...
    public static function calculatePriceGTMWithJumpDiffusion(
        ...
        $jumpSum = 0;
        for ($i = 1; $i <= $jumps; $i++) {
            $yi = $jumpSize + ($jumpControl * $z); // Random jump size
            $jumpSum += $jumpSize + $yi;
        }

        $e = $driftTerm + $diffusion + $jumpSum + $schock;

        $newPrice = $currentInvestmentPrice->value * exp($e);
        $lowerBound = 1;
